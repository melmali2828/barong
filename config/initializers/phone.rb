...
Barong::App.define do |config|
  ...
  Rails.application.config.to_prepare { config.write(:twilio_provider, TwilioSmsSendService) }
...
when 'twilio_sms'
  ...
  Rails.application.config.to_prepare { Barong::App.write(:twilio_provider, TwilioSmsSendService) }
...
when 'twilio_verify'
  ...
  Rails.application.config.to_prepare { Barong::App.write(:twilio_provider, TwilioVerifyService) }
when 'mock'
  if Rails.env.production?
    Rails.logger.info("WARNING! Don't use mock phone verification service in production")
  end
  Rails.application.config.to_prepare { Barong::App.write(:twilio_provider, MockPhoneVerifyService) }
